...
class UpgradePortal
{
    ...
    public function render_portal()
    {
        ...
        <style>
            ...
            .saps-search-input:focus {
                border-color: #0191FF;
                box-shadow: 0 4px 12px rgba(1, 145, 255, 0.1);
            }
        ...
        <div class="saps-upgrade-wrapper">
            ...
            <p style="color:#666; margin-bottom:40px;">Search for your school below to view your bundle pricing and receive an
                invoice.</p>
            ...
            <div id="saps-results-area">
                ...
                <div class="res-body">
                    ...
                    <p style="font-size:14px; color:#888; text-align:center;">Clicking below will email an invoice to the
                        address above.</p>
                    <button id="saps-confirm-btn" class="saps-btn" style="width:100%; background:#28a745;">✅ Confirm & Send
                        Invoice</button>
                </div>
            </div>

            <div id="saps-success-msg"
                style="display:none; margin-top:30px; background:#d4edda; color:#155724; padding:20px; border-radius:8px;">
            </div>
        </div>
    ...
    public function ajax_search_school()
    {
        ...
        if ($seed_post) {
            ...
            if (!$user) {
                ...
                if ($user_search)
                    $user = get_userdata($user_search->ID);
            }
            if ($user)
                $author_id = $user->ID;
        }
        ...
        if (!$author_id)
            wp_send_json_error("No schools found matching '$term'. Please try the exact School Name.");
        ...
        if (empty($listings))
            wp_send_json_error("Author found, but no listings attached.");
        ...
        if (!$calc['ok']) {
            wp_send_json_error("Pricing Error: " . $calc['error']);
        }
