 ...
 * Description: Standalone invoice viewer at /invoice/ requiring signed link. Allows inline invoice detail edits via signed save.
 * Version: 1.0.0
 */
...
class NGD_Standalone_Invoice_Signed {
    ...
    private static function build_invoice_data($listing_id, $ref, $exp, $sig) {
        ...
        // Pull billing details from USER meta (preferred)
    ...
    private static function render_invoice_page($inv) {
        ...
        // Full standalone HTML document (no theme)
        ...
        echo '  <div class="info">';
        echo '    <div class="box">';
        echo '      <div class="row"><span class="k">Client Name:</span><span class="v">' . esc_html($inv['client_name']) . '</span></div>';
        echo '      <div class="row"><span class="k">Contact person:</span><span class="v editable" contenteditable="true" data-key="contact_person">' . esc_html($inv['contact_person']) . '</span></div>';
        echo '      <div class="row"><span class="k">School Reg No:</span><span class="v editable" contenteditable="true" data-key="client_reg">' . esc_html($inv['client_reg']) . '</span></div>';
        echo '      <div class="row"><span class="k">VAT Number:</span><span class="v editable" contenteditable="true" data-key="client_vat">' . esc_html($inv['client_vat']) . '</span></div>';
        echo '    </div>';

        echo '    <div class="box">';
        echo '      <div class="row"><span class="k">Date:</span><span class="v">' . esc_html($inv['issued_date']) . '</span></div>';
        echo '      <div class="row"><span class="k">Email:</span><span class="v editable" contenteditable="true" data-key="client_email">' . esc_html($inv['client_email']) . '</span></div>';
        echo '      <div class="row"><span class="k">Address:</span><span class="v editable" contenteditable="true" data-key="client_address">' . nl2br(esc_html($inv['client_address'])) . '</span></div>';
        echo '    </div>';
        echo '  </div>';
              ...
              saveBtn.addEventListener("click", function(){
                ...
                post({
                  ...
                  if(resp && resp.success){
                    ...
                    setTimeout(function(){ saveBtn.innerText = "Save details"; saveBtn.disabled = false; }, 1200);
                  } else {
    ...
    public static function ajax_save_invoice_details() {
        ...
        // Optional: also mirror to listing meta for compatibility
...
.row .v{flex:1}
.editable{min-height:18px;padding:2px 4px;border-radius:8px;outline:none}
.editable:focus{box-shadow:0 0 0 3px rgba(11,102,255,.18);background:#fbfdff}
