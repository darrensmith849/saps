...
class NGD_Standalone_Invoice_Signed
{
    ...
    private static function build_invoice_data($listing_id, $author_id, $ref, $exp, $sig)
    {
        ...
        // Resolve School Name (Override > Meta > Title)
        $override_name = '';
        if ($author_id) {
            $override_name = get_user_meta($author_id, 'ngd_invoice_school_name_override', true);
        }
        if (!$override_name) {
            $override_name = get_post_meta($listing_id, '_ngd_invoice_school_name_override', true);
        }
